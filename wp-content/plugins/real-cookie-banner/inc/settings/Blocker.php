...
class Blocker {
    ...
    public function revisionArray($result) {
        ...
        foreach ($blockers as $blocker) {
            ...
            $nonVisual[] = [
                'id' => $blocker->ID,
                self::META_NAME_HOSTS => $blocker->metas[self::META_NAME_HOSTS],
                self::META_NAME_COOKIES => $blocker->metas[self::META_NAME_COOKIES]
            ];
        }
    ...
    public function deleted_post($postId) {
        ...
        if ($post_type === \DevOwl\RealCookieBanner\settings\Cookie::CPT_NAME) {
            $blockers = $this->getOrdered(
                \false,
                get_posts(
            ...
            foreach ($blockers as $blocker) {
                ...
                if (($key = \array_search($postId, $cookies, \true)) !== \false) {
                    unset($cookies[$key]);
                    update_post_meta($blocker->ID, self::META_NAME_COOKIES, \join(',', $cookies));
                }
            }
        }
