...
class MyConsent {
    ...
    public function persist(
        ...
        $wpdb->query(
            ...
            \str_ireplace(
                ...
                'NULL',
                $wpdb->prepare(
                    "INSERT IGNORE INTO {$table_name}\n                        (ipv4, ipv6, ipv4_hash, ipv6_hash, uuid, revision, revision_independent, previous_decision, decision, decision_hash, blocker, dnt, button_clicked, context, viewport_width, viewport_height, referer, url_imprint, url_privacy_policy, forwarded, forwarded_blocker, created)\n                        VALUES\n                        (%d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %s, %d, %d, %s, %s, %s, %s, %s, %s)",
         ...
         * @param {array} $result
         */
        do_action('RCB/Consent/Created', $result);
        return $result;
    }
    /**
    ...
    public function setCookie($uuid = null, $revision = null, $consent = null) {
        ...
        $set = \DevOwl\RealCookieBanner\Utils::setCookie(
            $cookieName,
            $cookieValue,
            $expire,
            \constant('COOKIEPATH'),
     ...
     */
    protected function getCurrentUser($force = \false) {
    ...
    public function getCurrentHistory() {
        ...
        if ($user !== \false) {
            ...
            $rows = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT uc.id, uc.created, uc.decision, r.json_revision, uc.dnt, uc.blocker, uc.forwarded\n                    FROM {$table_name} uc\n                    INNER JOIN {$table_name_revision} r\n                    ON r.hash = uc.revision\n                    WHERE uuid = %s\n                    ORDER BY uc.created DESC\n                    LIMIT 0, 100",
                    $user['uuid']
                ),
                ARRAY_A
            );
    ...
    public function getCookieName() {
        $implicitString = \DevOwl\RealCookieBanner\settings\Revision::getInstance()->getContextVariablesString(\true);
