    ...
    protected function getExportBlocker() {
        ...
        foreach ($posts as $post) {
            ...
            foreach (self::RELEVANT_BLOCKER_POST_KEYS as $key) {
                ...
                // Cookies should be resolved as post_name instead of ID
                if ($key === 'metas') {
                    foreach (
                        $rowResult[$key][\DevOwl\RealCookieBanner\settings\Blocker::META_NAME_COOKIES]
                        as $cookieIdx => $cookieId
                    ) {
                        $rowResult[$key][\DevOwl\RealCookieBanner\settings\Blocker::META_NAME_COOKIES][
                            $cookieIdx
                        ] = get_post($cookieId)->post_name;
                    }
                }
            }
            $result[] = $rowResult;
